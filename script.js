(function() {
    ...
    if (rotatingEl) {
        ...
        if (rotatingEl.dataset.phrases) {
            try { phrases = JSON.parse(rotatingEl.dataset.phrases); } catch(e) { phrases = ["Ingeniería de vanguardia", "Construcción inteligente", "Software que transforma"]; }
        } else { phrases = ["Ingeniería de vanguardia", "Construcción inteligente", "Software que transforma"]; }
        ...
        function typeEffect() {
            ...
            if (!isDeleting) {
                ...
                else { typeSpeed = 80 + Math.random() * 40; }
            } else {
                rotatingEl.textContent = current.substring(0, charIndex);
                charIndex--;
                if (charIndex < 0) { isDeleting = false; phraseIndex = (phraseIndex + 1) % phrases.length; typeSpeed = 300; }
                else { typeSpeed = 40 + Math.random() * 30; }
            }
    ...
    // --- Intersection Observer (animaciones y contadores) ---
